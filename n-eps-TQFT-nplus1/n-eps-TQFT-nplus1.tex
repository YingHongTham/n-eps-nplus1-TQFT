...
See \figref{f:defn-1-hdl-alt}.

However, it turns out this is not enough,
essentially because of symmetry.
In \figref{f:defn-1-hdl-alt}, we see that we could have defined
...
$b: B \to B'$ in $\cB$,
we see that the assignment from \rmkref{r:profunctor-alt},
...
forces the tensor product in \defref{d:hatbox}.
\end{remark}

Let us further unpack \defref{d:profunctor-vec}.
...
\begin{remark}
We saw from \xmpref{x:profunctor-bimodule}
that $F: *//A$
\end{remark}
...
%$\Gamma \sqcup \Gamma'
%\in \Skein(N \times I \sqcup N \times I;
%\ov{\VV} \sqcup \VV' \sqcup \ov{\VV'} \sqcup \VV'')$,
%glued along the common $\VV'$ boundary value,
%is $\Gamma' \circ \Gamma$.
...
where $M: \empt{0} \to (+,-)$ is from \xmpref{x:cup-cap},
...
where $M': (+,-) \to \empt{0}$ is from \xmpref{x:cup-cap},
and we have identifications of the incoming/outoing boundaries,
...
repeated applications of \lemref{l:id-profunctor}.
From \prpref{p:1-cobord-compose},
